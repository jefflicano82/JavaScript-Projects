...
let activePlayer = "X";
//This array stores an array of moves. We use this to determin ewin conditions.
let selectedSquares = [];

//This function is for placing an x or an o into a square.
function placeXOrO(squareNumber) {
  //This condition ensures a square hasnt been selected already.
  //The .some()method is used to check each element of teh selectSquare array
  ...
  if (!selectedSquares.some((element) => element.includes(squareNumber))) {
    ...
    //drawline() function is called the draw a line on the screen if the condition is met.
    ...
    checkWinConditions();
    //This conditions if for chaning the active player
    if (activePlayer === "X") {
    ...
    //This conditions checks to see if it is the computers turn.
    if (activePlayer === "O") {
  ...
  function computersTurn() {
    ...
    let pickASquuare;
    //This conditions allows our while loop to keep trying if a square is slected already.
    while (!success) {
      //A random number between 0 and 8 is selected.
      pickASquuare = String(Math.floor(Math.random() * 9));
      //If the random number evaluated returns true, the square hasnt been selected yet.
      if (placeXOrO(pickASquare)) {
        //This line calls the function/
        placeXOrO(pickASquuare);
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
  ...
  const canvas = document.getElementById("wing-linse");
  ...
  let x1 = coordX1,
    //This line indicates where the start of a linse y axis is.
    x2 = coordX2,
    //This line inidcates where the end of lines x axis is.
    y2 = coordY2,
    //This variable stores temporary x axis data we update in our animation loop.
    x = x1,
    //This variables stores temproary y axis data we update in our animation loop.
    y = y1;
  //This function interacts with the canvas.
  function animateLineDrawing() {
    //This variable creates a loop.
    const animationLoop = requestAnimationFrame(animateLineDrawing);
    //This method clears content from the lats loop iteration.
  ...
  disableClick();
  //this lin eplays the win sounds.
  audio("./media/winGame.mp3");
  //This lin ecalls our main animation loop.
  animateLineDrawing();
